...
def main():
    ...
    if username and token and button_pressed:
        ...
        if "errors" in cont_data:
            error_messages.append(f"Contribution data: {cont_data['errors']}")
        if "errors" in user_data:
            error_messages.append(f"User data: {user_data['errors']}")
        if "errors" in repo_data:
            error_messages.append(f"Repository data: {repo_data['errors']}")
        ...
        error_messages = []
        if "errors" in cont_stats:
            error_messages.append(f"Processing contribution data: {cont_stats['errors']}")
        if "errors" in user_stats:
            error_messages.append(f"Processing user data: {user_stats['errors']}")
        if repo_stats is None:
            error_messages.append("Failed to process language data")
